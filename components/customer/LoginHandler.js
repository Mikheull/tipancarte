    ...
    commerce.customer.getToken(token)
      ...
      .catch(() => {
        this.setState({
          loading: false,
          isError: true,
          message: ['Le lien de connexion à expirer. Veuillez réessayer.'],
        });
      });
  }
    ...
    return commerce.customer.login(
      ...
      .then(() => {
        this.setState({
          ...
          email: '',
          message: [
            'Si vous avez déjà passé une commande chez nous, nous allons vous envoyer un email avec un lien de connexion!'
          ]
        });
      })
      .catch((error)=>{
