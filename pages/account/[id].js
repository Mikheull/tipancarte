...
export default function SingleOrderPage() {
  ...
  const OrderDate = ({ date: data }) => {
    ...
    return (
      <small><strong>Commande passée le:</strong> { moment(date).format('MMM Do Y') }</small>
    )
  };
   ...
   */
  if (error) {
    return <TemplatePage page={ {message: 'Une erreur est survenue.'} } />
  }
   ...
   */
  if (!data) {
    return <TemplatePage page={ {message: 'Désolé, nous ne trouvons pas votre commande, si vous pensez qu\'il s\'agit d\'une erreur merci de nous contacter !'} } />
  }
